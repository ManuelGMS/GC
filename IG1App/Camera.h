...
public:
	...
	*/
	...
	// Para realizar con la cámara una trayectoria circular alrededor de look.
...
protected:
	...
	// Atributos para gestionar el radio y el ángulo de la circunferencia (Orbit)
	GLdouble mAng; // Ángulo (distancia angular) de la cámara (desde el eje X positivo 0°, sentido antihorario).
	GLdouble mRadio; // Radio de una esfera imaginaria sobre el cual se mueve la cámara.
	
	/*
	Los ejes(U, V, N) definen el sistema de coordenadas de la cámara, se obtienen de las filas (row: 0, 1, 2) de mViewMat.
	Ejes / vectores son mRight(eje X), mUpward(ejeY) y mFront(este último es hacia donde mira la cámara(-Z "o" -N)).
	*/
